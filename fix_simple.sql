-- ═══════════════════════════════════════════════════════════════════════════════
--  ClassExpress — Database Fix Script (Simplified)
--  Solo agrega columnas faltantes sin foreign keys complejos
-- ═══════════════════════════════════════════════════════════════════════════════
...
ALTER TABLE sesiones_clase 
    ADD COLUMN IF NOT EXISTS duracion_min INT DEFAULT 0,
...
-- Agregar columnas faltantes a paises si ya existe
ALTER TABLE paises 
    ADD COLUMN IF NOT EXISTS codigo_iso VARCHAR(3) NOT NULL AFTER nombre,
    ADD COLUMN IF NOT EXISTS created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP AFTER codigo_iso;

-- Insertar países solo si la tabla está vacía
...
-- ─────────────────────────────────────────────────────────────────────────────
--  9. Actualizar usuarios para agregar username único
-- ─────────────────────────────────────────────────────────────────────────────
